...
async function start() {
  ...
  const postCreate = await trpc.posts.create.mutate({title: "hehe"});

  const postList2 = await trpc.posts.list.query();
  console.log('>>> anon:posts:list:', postList2);

  await trpc.posts.reset.mutate();

  const postList3 = await trpc.posts.list.query();
  console.log('>>> anon:posts:list:', postList3);
